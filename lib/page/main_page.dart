...
class _MainFlamePageState extends State<MainFlamePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Call fetchMarkers and update NavPointManager
      final provider = Provider.of<RosChannel>(context, listen: false);
      provider.fetchMarkers().then((points) {
          ...
          Card(
            ...
            child: Consumer<GlobalState>(
              builder: (context, globalState, child) {
                return IconButton(
                  onPressed: () {
                    globalState.toggleLayer('showLocalCostmap');
                    setState(() {});
                  },
                  icon: Icon(
                    globalState.isLayerVisible('showLocalCostmap')
                        ? Icons.map
                        : Icons.map_outlined,
                    color: globalState.isLayerVisible('showLocalCostmap')
                        ? Colors.green
                        : theme.iconTheme.color,
                  ),
                  tooltip: '切换膨胀层显示',
    ...
    return Positioned(
      ...
      child: Row(
        ...
        children: [
          ...
          Card(
            elevation: 10,
            child: Consumer<NavPointManager>(
              builder: (context, manager, child) {
                Set<String> types = {'All'};
                for (var point in manager.navPoints) {
                ...
                return PopupMenuButton<String>(
                  ...
                  onSelected: (String result) {
                    setState(() {
                      game.setPointTypeFilter(result);
                    });
                  },
                  itemBuilder: (BuildContext context) => types.map((String type) {
                    return PopupMenuItem<String>(
                      value: type,
                      child: Text(type == 'All' ? '全部点位' : _getTypeText(type)),
                    );
                  }).toList(),
                );
              },
